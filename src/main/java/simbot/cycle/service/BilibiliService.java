...
//            }
    ...
    public MessagesBuilder parseDynamicSvrBody(BilibiliDynamicSvrCardsInfo dynamicSvrCardsInfo) throws IOException {
        ...
        if (type == 8) {
            ...
            //视频图片
            Resource videoImgInfo = parseUserPic(cardInfo.getPic(), null, null);
            //头像
            Resource userImgInfo = parseUserPic(userInfo.getFace(), 50, 50);

            //转化为messageChain
            builder = parseDynamicSvrMessageChain(cardInfo.getTitle(),
                    videoImgInfo,
                    cardInfo.getDesc(),
                    userImgInfo,
                    userInfo.getUname(),
            ...
            //视频图片
            Resource videoImgInfo = parseUserPic(cardInfo.getCover(), null, null);
            //头像
            Resource userImgInfo = parseUserPic(apiSeasonInfo.getCover(), 50, 50);

            //转化为messageChain
            builder = parseDynamicSvrMessageChain(cardInfo.getNewDesc(),
                    videoImgInfo,
                    null,
                    userImgInfo,
    ...
    private MessagesBuilder parseDynamicSvrMessageChain(String title,
                                                     Resource videoImgInfo,
                                                     String desc,
                                                     Resource userImgInfo,
                                                     String uName,
                                                     Long uid,
                                                     Date createTime,
                                                     String link) {
        ...
        builder.text("[" + uid + "]\n");
        //投稿时间
        builder.text("[" + DateUtil.toString(createTime) + "]\n");
        //短连接
        builder.text("[" + link + "]");
    ...
    //处理头像图片大小
    private Resource parseUserPic(String imgUrl, Integer width, Integer height) throws IOException {
        //先把图片下载下来
