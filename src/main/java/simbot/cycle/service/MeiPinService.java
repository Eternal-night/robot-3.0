...
import simbot.cycle.entity.MeiPinInfo;
...
public class MeiPinService {
     ...
     */
    public MessagesBuilder lastMeiPinMsgChain() throws IOException {
     ...
     */
    public MessagesBuilder parseMsg(MeiPinInfo meiPinInfo) {
        MessagesBuilder builder = new MessagesBuilder();

        //先放上4chan的logo
        Resource image = rabbitBotService.uploadMiraiImage(logo_4chan_path);
        builder.image(image).text("\n")
                //然后是标题，文章链接
                .text(meiPinInfo.getTitle()+"\n")
                .text(meiPinInfo.getUrl()+"\n")
                .text("===========================\n")
                .text("转载只图一乐，不代表任何政治立场\n")
                .text("小盆友们请在有家长陪同的情况下观看\n");
