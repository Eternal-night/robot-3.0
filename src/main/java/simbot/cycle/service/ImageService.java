...
public class ImageService {
    ...
    public String getGuguguRandom() {
        ...
        if (StringUtil.isBlank(guguguImageFullName)) {
            return null;
        }
    ...
    public SaucenaoSearchInfoResult searchImgFromSaucenao(String imgUrl) throws CycleException {
        ...
        try {
            ...
            if (null == saucenaoSearchResult) {
                throw new CycleException(ConstantImage.SAUCENAO_API_SEARCH_FAIL);
            }
            ...
            if (null == infoResultList || infoResultList.size() <= 0) {
                throw new CycleException(ConstantImage.SAUCENAO_SEARCH_FAIL);
            }
            ...
            throw new CycleException(ConstantImage.SAUCENAO_API_TIMEOUT_FAIL);
        } catch (Exception ex) {
            logger.error(ConstantImage.SAUCENAO_API_REQUEST_ERROR + ex.toString(), ex);
            throw new CycleException(ConstantImage.SAUCENAO_API_REQUEST_ERROR);
        }
        return null;
    }
